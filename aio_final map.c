...
/*   Updated: 2025/04/08 11:00:12 by kearmand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	free_state_list(t_state *state);
void	add_state(t_state **state, t_state *new);
...
void actualise_depth(t_state *next)
{
	...
	//Je veux stocker la range de la depth
	int size = 0;
	int j;
	for (j = 0; j < g_max_depth; j++)
	{
		if (next->depth[j] != 0)
		{
			break ;
	...
	if (size > g_max_width_dept)
	{
		g_max_width_dept = size;
		//afficher la map
		printf("\nmap : %o\n", next->id);
		printf("size : %d =>\n", size);
		for (int i = 0; i <= g_max_depth; i++)
		{
			printf("%d ", next->depth[i]);
		}
		printf("\n");
	}
}
